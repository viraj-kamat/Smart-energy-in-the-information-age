...
def read_data(home_name) :
    ...
    for row in reader :                     # Read the meter dataframe, modify timestamps such that it can be merged with the weather data.
        ...
        if new_row['Date & Time'].minute == 30 and home_name in ['b'] :
            continue
        elif new_row['Date & Time'].minute not in [0,30] and home_name in ['c'] :
            continue
    ...
    if home_name in ["f","c"] :
        ...
        meter_data = meter_data.resample('H').sum()                 #Resample the data such that all the minute values are merged into an hour
...
#Run the models for each of the houses
models = ["linear","xgb","randomforest","adaboost","svm","xgb","gradientboost","lstm"] # 
houses = ["b","c","f"]
for house in houses :
    for model in models :
        train_model(house,model)
        print("\n")